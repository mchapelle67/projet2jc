...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function validateStep(stepIndex) {
        ...
        if (dateInput) {
            ...
            const errorMsg = document.createElement('p');
            errorMsg.className = 'error-message';
            errorMsg.textContent = 'Veuillez saisir une date.';
    ...
    function populateReviewDetails() {
        ...
        const reviewData = {
            ...
            marque: formData.get('rdv_type_form[vehicule][marque]') || 'Non renseigné',
            modele: formData.get('rdv_type_form[vehicule][modele]') || 'Non renseigné',
    ...
    // gestion du bouton "Suivant" pour valider l'étape actuelle
    nextBtn.addEventListener('click', () => {
        if (validateStep(currentStep)) {
            currentStep++;
            // Appelle le calendrier à l'étape 3 (index 2)
            if (currentStep === 2 && typeof initCalendarWidget === "function") {
                initCalendarWidget();
            }
